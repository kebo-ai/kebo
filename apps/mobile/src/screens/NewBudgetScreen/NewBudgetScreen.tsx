...
import { CalendarRangePicker } from "../../components/custom/CalendarRangePicker";
import { ArrowDownSimpleIcon } from "../../components/svg/ArrowDownSimpleIcon";

interface NewBudgetScreenProps extends AppStackScreenProps<"NewBudget"> {
  navigation: AppStackScreenProps<"NewBudget">["navigation"];
}
    ...
    async (values: { name: string; startDate: Date; endDate: Date }) => {
      try {
        ...
        if (isEditing && budgetId) {
          ...
          if (result?.id) {
            navigation.replace("Budget", { budgetId: result.id });
          } else {
            showToast("error", translate("newBudgetScreen:errorMessage"));
          }
        }
