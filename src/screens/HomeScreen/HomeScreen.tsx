...
import { SpentSvg } from "../../components/svg/SpentSvg";
import { TranferSvg } from "../../components/svg/TranferSvg";
import { IncomeSvg } from "../../components/svg/IncomeSvg";
import { NewAccountSvg } from "../../components/svg/NewAccountSvg";
import { KebitoWiseBudgetSvg } from "../../components/svg/KebitoWiseBudgetSvg";
import { KebitoWiseSuggestsSvg } from "../../components/svg/KebitoWiseSuggestsSvg";
import { KebitoWiseProjectSvg } from "../../components/svg/KebitoWiseProjectSvg";
...
import { RecurrenceIconHomeSvg } from "../../components/svg/RecurrenceIconHomeSvg";
import { translateCategoryName } from "../../utils/categoryTranslations";
import { useReviewModal } from "../../hooks/useReviewModal";
import CustomModalReview from "../../components/custom/CustomModalReview";
...
export const HomeScreen: FC<HomeScreenProps> = observer(function HomeScreen({
  ...
  useFocusEffect(
    useCallback(() => {
      const checkReviewModal = async () => {
        // Check if user is coming from creating a transaction
        // This can be done by checking for a flag in the navigation state
        // or using a navigation parameter
        const navigationState = navigation.getState();
        const currentRoute = navigationState.routes[navigationState.index];

        // Only check if coming from transactions screen
        if (currentRoute.name === "Home") {
          // Check if there's a parameter indicating a transaction was created
          const params = currentRoute.params as any;
          if (params?.transactionCreated) {
            setTimeout(async () => {
  ...
  const navigateToTransaction = useCallback(
    (type: TransactionType) => {
      ...
      navigation.navigate("Transaction" as any, { transactionType: type });
    },
    [navigation]
  );
  ...
  const navigateToSelectBank = useCallback(() => {
    ...
    navigation.navigate("Accounts" as any, { visible: true });
  }, [navigation]);
  ...
  const navigateToChatbot = useCallback(
    (initialQuestionKey: KeboWiseKeys) => {
      ...
      navigation.navigate("Chatbot", { initialQuestion });
    },
    [navigation]
  );
  ...
  const handleSeeMoreTransactions = useCallback(() => {
    ...
    navigation.navigate("Transactions", { origin: "Home" } as any);
  }, [navigation]);
